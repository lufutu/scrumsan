import { NextRequest, NextResponse } from 'next/server'
import { createClient } from '@/lib/supabase/server'
import { prisma } from '@/lib/prisma'
import { getCurrentUser } from '@/lib/auth-utils'

export async function GET(
  req: NextRequest,
  { params }: { params: Promise<{ slug: string; boardSlug: string }> }
) {
  try {
    const supabase = await createClient()
    const user = await getCurrentUser(supabase)
    const { slug: orgSlug, boardSlug } = await params
    const url = new URL(req.url)
    const includeDetails = url.searchParams.get('includeDetails') === 'true'

    // 1. First resolve organization by slug
    const organization = await prisma.organization.findUnique({
      where: { slug: orgSlug },
      select: { id: true }
    })

    if (!organization) {
      return NextResponse.json(
        { error: 'Organization not found' },
        { status: 404 }
      )
    }

    // 2. Check if user is a member of the organization
    const orgMember = await prisma.organizationMember.findFirst({
      where: {
        organizationId: organization.id,
        userId: user.id
      }
    })
    
    if (!orgMember) {
      return NextResponse.json(
        { error: 'Unauthorized' },
        { status: 403 }
      )
    }

    // 3. Get board by organization + slug
    const board = await prisma.board.findUnique({
      where: {
        organizationId_slug: {
          organizationId: organization.id,
          slug: boardSlug
        }
      },
      select: { id: true }
    })

    if (!board) {
      return NextResponse.json(
        { error: 'Board not found' },
        { status: 404 }
      )
    }

    // 4. Get sprints for this board (same logic as /api/sprints?boardId=)
    const sprintsQuery = {
      where: { boardId: board.id },
      include: includeDetails ? {
        tasks: {
          include: {
            assignees: {
              include: {
                user: {
                  select: {
                    id: true,
                    fullName: true,
                    email: true,
                    avatarUrl: true
                  }
                }
              }
            },
            reviewers: {
              include: {
                user: {
                  select: {
                    id: true,
                    fullName: true,
                    email: true,
                    avatarUrl: true
                  }
                }
              }
            },
            taskLabels: {
              include: {
                label: {
                  select: {
                    id: true,
                    name: true,
                    color: true
                  }
                }
              }
            }
          }
        }
      } : undefined,
      orderBy: { createdAt: 'desc' as const }
    }

    const sprints = await prisma.sprint.findMany(sprintsQuery)

    // Transform sprint tasks if included
    const transformedSprints = sprints.map(sprint => ({
      ...sprint,
      tasks: sprint.tasks?.map(task => ({
        ...task,
        assignees: task.assignees?.map((a: any) => a.user) || [],
        reviewers: task.reviewers?.map((r: any) => r.user) || [],
        labels: task.taskLabels?.map((tl: any) => tl.label) || []
      })) || undefined
    }))

    return NextResponse.json(transformedSprints)

  } catch (error: unknown) {
    console.error('Error fetching sprints:', error)
    return NextResponse.json(
      { error: (error as Error).message || 'Failed to fetch sprints' },
      { status: 500 }
    )
  }
}