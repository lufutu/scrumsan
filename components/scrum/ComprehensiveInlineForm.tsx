...
export function ComprehensiveInlineForm({
  ...
  const getFilteredItems = () => {
    const allItems = [
      ...
      // Assignees
      ...users.map(user => ({ ...user, category: 'Assignees', itemType: 'assignee' as const })),
      // Labels
      ...labels.filter(label => !selectedLabels.find(l => l.id === label.id))
          ...
          {Object.entries(groupedItems).map(([category, items]) => (
            <div key={category}>
              ...
              {items.map((item) => (
                <button
                  key={item.id}
                  onClick={() => handleItemSelect(item, item.itemType)}
